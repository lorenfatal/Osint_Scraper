...
# Parser for Google cloud site, inheriting from ParserBase
class CloudGoogleParser(ParserBase):

    # Checks if this parser can handle the given URL
    def can_handle(self, url):
        domain = urlparse(url).netloc.lower()
        return domain.endswith('cloud.google.com')
    ...
    def fetch_data(self, url):
        try:
            ...
            ## TITLE
            # Find title
            current_class = 'Qwf2Db-MnozTc Qwf2Db-MnozTc-OWXEXe-MnozTc-ibL1re'
            ...
            content = ''
            unwanted_headers = ['IOCs', 'Indicators of Compromise', 'Indicators of Compromise (IOCs)', 'Acknowledgements', 'Detections', 'YARA Rules', 'Network-Based IOCs', 'Host-Based IOCs', 'Related articles']
            header_tags = ['h2', 'h3', 'h4', 'h5']

            # Find all header tags
            for header in article_content.find_all(header_tags):
                ...
                if current_title not in unwanted_headers:
                    ...
                    while next_node and next_node.name not in header_tags:
                        if next_node.name == 'p':
...
register_parser(CloudGoogleParser())